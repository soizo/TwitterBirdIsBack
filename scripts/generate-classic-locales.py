...
WORD = re.compile(
    r"\b(?:reposting|reposted|reposts|repost|posting|posted|posts|post)\b", re.I
)
STRING = r"""(?:"(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*')"""
...
def fetch(url):
    if not re.fullmatch(
        r"https://abs\.twimg\.com/responsive-web/client-web/i18n/[a-zA-Z-]+\.[a-f0-9]+\.js",
        url,
    ):
    ...
    messages = {
        m[1]: parse_json(m[2])
        for m in re.finditer(r'\b\w+\("([a-z0-9]{8})",("(?:\\.|[^"\\])*")\)', source)
    }
...
def template(source, identifier, category="other"):
    ...
    match = re.search(
        r'\("' + re.escape(identifier) + r'",\(?function\((\w+)\)\{return([^}]+)\}',
        source,
    )
    if not match:
    ...
    while rest:
        ...
        if string:
            ...
            result += {"fullName": "{name}", "tweetText": "{text}", "count": "{count}"}[
                field[1]
            ]
            rest = rest[field.end() :]
        elif plural:
            ...
            index = (
                (0 if category == "one" else 1)
                if len(choices) == 2
                else {"few": 0, "many": 1, "one": 2, "other": 3}[category]
            )
...
def unique_pairs(pairs):
    ...
    return {
        ...
        for current, values in candidates.items()
...
def build(resources):
    ...
    notice_section = english_source.split("const notificationMessages = new Set(", 1)[
        1
    ].split("].map(notificationKey)", 1)[0]
    notice_text = {
        parse_json(m[0]) for m in re.finditer(r'"(?:\\.|[^"\\])*"', notice_section)
    }
    ...
    for language, versions in resources.items():
        ...
        for key in ["a50c911e", "ie08d3a4"]:
            current = new[key]
            restored = current.replace(noun, old_noun).replace(
                noun.lower(), old_noun.lower()
            )
            if language.startswith("en"):
                restored = WORD.sub(lambda m: TERMS[m[0].lower()], current)
            labels.append((current, restored))
            notices.append((current, restored))
        actions = unique_pairs(
            [
                (new["df34a454"], old["bea869b4"]),
                (new["f2919fb8"], old["d6c8514a"]),
                (new["fd1e5446"], old["f3bbbb88"]),
                (new["b8c465e2"], old["g23ce6f0"]),
            ]
        )
        headings = unique_pairs(
            [
                (noun, old_noun),
                (new["fa4e68ca"], old["bab1f8b0"]),
                (new["ja42739e"], old["d497b854"]),
            ]
        )
        counts = []
        for current, historical, action in [
            ("dfad425d", "i769b0ab", "d6c8514a"),
            ("a386dc55", "ea9a1f0d", "g23ce6f0"),
        ]:
            for category in CATEGORIES:
                ...
                after = template(
                    old_source, "ea9a1f0d" if language == "uk" else historical, category
                )
        ...
        result[language] = {
            ...
            "labels": unique_pairs(labels),
            "notices": unique_pairs(notices),
            "counts": counts,
            "title": [title_before, title_after],
        }
        print(
            language,
            len(result[language]["labels"]),
            "labels,",
            len(result[language]["notices"]),
            "distinct notices",
            flush=True,
        )
    return result
...
def main():
    ...
    sources = {
        language: {
            ...
            "historical": f"https://abs.twimg.com/responsive-web/client-web/i18n/{language}.{version}.js",
        }
        for language, version in HISTORICAL.items()
    }
    jobs = [
        (language, version, url)
        for language, versions in sources.items()
        for version, url in versions.items()
    ]
    resources = {language: {} for language in HISTORICAL}
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as pool:
        for (language, version, _), data in zip(
            jobs, pool.map(fetch, [job[2] for job in jobs]), strict=True
        ):
...
if __name__ == "__main__":
    ...
    assert (
        template(
            'o("12345678",function(e){return e.count+" post"+r(e.count,"","s")})',
            "12345678",
            "one",
        )
        == "{count} post"
    ...
    assert (
        template(
            'o("12345678",function(e){return e.fullName+" / "+e.tweetText})', "12345678"
        )
        == "{name} / {text}"
    )
    try:
