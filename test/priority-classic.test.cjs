...
async function inject(page) {
  ...
  for (const file of manifest.content_scripts[0].js.filter(
    (file) => file !== "settings.js",
  )) {
    await page.addScriptTag({
      path: path.resolve(__dirname, "../extension", file),
    });
  }
}
...
test("Simplified Chinese restores sourced controls and split notifications without changing user content", async (t) => {
  ...
  assert.equal(
    await page.locator("#post svg").count(),
    1,
    "icons must not be flattened",
  );
  assert.equal(await page.locator("h2").textContent(), "推文");
  assert.equal(await page.locator("#undo").textContent(), "撤销转推");
  assert.equal(
    await page.locator("#count").getAttribute("aria-label"),
    "7 转推。转推",
  );
  assert.equal(await page.locator("#notice").textContent(), "你的推文已发送。");
  assert.equal(
    await page.locator("#unknown-notice").textContent(),
    "帖子 X 的未知通知",
  );
  assert.equal(
    await page.locator("#unknown-control").textContent(),
    "Post Malone",
  );
  assert.equal(
    await page.locator("#user-text").textContent(),
    "帖子 转帖 Post X",
  );
  ...
  assert.equal(
    await page.title(),
    '(8) Post Malone 在 Twitter: "X 帖子" / Twitter',
  );
  await page.locator("#view").click();
  assert.equal(await page.locator("body").getAttribute("data-clicked"), "yes");
});

test("priority locales preserve nouns, verbs, counted states, title grammar and dynamic updates", {
  timeout: 60000,
}, async (t) => {
  const browser = await chromium.launch({ channel: "chrome", headless: true });
  t.after(() => browser.close());
  ...
  for (const fixture of require("./fixtures/priority-classic.json")) {
    ...
    await page.evaluate((f) => {
      ...
      for (const key of ["heading", "undo", "notice"])
        document.querySelector("#" + key).textContent = f[key][0];
    ...
    await inject(page);
    for (const key of ["post", "heading", "undo", "notice"])
      assert.equal(
        await page.locator("#" + key).textContent(),
        fixture[key][1],
        fixture.language + ":" + key,
      );
    assert.equal(
      await page.locator("#tab").textContent(),
      fixture.heading[1],
      fixture.language + ":tab noun",
    );
    assert.equal(
      await page.locator("#tooltip").textContent(),
      fixture.undo[1],
      fixture.language + ":tooltip",
    );
    assert.equal(
      await page.locator("#post").getAttribute("title"),
      fixture.post[1],
    );
    assert.equal(
      await page.locator("#count").getAttribute("aria-label"),
      fixture.count[1],
      fixture.language + ":count",
    );
    assert.equal(
      await page.title(),
      "(2) " + fixture.title[1],
      fixture.language + ":title",
    );
...
test("unimplemented languages keep their wording, late language selection works, and profile headings remain names", async (t) => {
  ...
  await page.route("https://x.com/**", (route) =>
    route.fulfill({
      contentType: "text/html; charset=utf-8",
      body: `<!doctype html><html lang="fr"><head><title>Post / X</title></head><body>
  ...
  assert.equal(
    ...
    "Post Malone sent a post.",
  );
  assert.equal(
    await page.title(),
    "Post / Twitter",
    "generic site branding must not regress outside the priority locales",
  );
  await page.evaluate(() => {
    document.documentElement.lang = "ja";
    document.querySelector("#post").textContent = "ポスト";
  });
  assert.equal(await page.locator("#post").textContent(), "ツイートする");
  assert.equal(
    await page.locator("h2").textContent(),
    "ポスト",
    "a profile display name is not a detail heading",
  );
  for (const name of ["constructor", "__proto__", "toString"]) {
    await page.evaluate((name) => {
      document.title = name + " / X";
    }, name);
    assert.equal(
      await page.title(),
      name + " / Twitter",
      "object property names are still user text",
    );
  }
  for (const [code, cases] of [
    [
      ...
      [
        ...
        ["2", "репоста", "ретвита"],
        ["5", "репостов", "ретвитов"],
        ["11", "репостов", "ретвитов"],
        ["21", "репост", "ретвит"],
        ["1 002", "репоста", "ретвита"],
      ],
    ],
    [
      "uk",
    ...
    for (const [count, current, classic] of cases) {
      await page.evaluate(
        ({ code, count, current }) => {
          ...
          document
            ...
            .setAttribute(
              "aria-label",
              `${count} ${current}. ${code === "ru" ? "Сделан репост" : "Зроблено репост"}`,
            );
        },
        { code, count, current },
      );
      assert.equal(
        await page.locator("#count").getAttribute("aria-label"),
        `${count} ${classic}. ${code === "ru" ? "Ретвитнуто" : "Ретвітнуто"}`,
      );
    }
  }
});
