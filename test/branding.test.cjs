...
test("extension branding uses Twitter wording and complete square icon assets", () => {
  ...
  vm.runInContext(
    fs.readFileSync(path.join(extension, "popup-locales.js"), "utf8"),
    context,
  );
  const ownCopy = [
    manifest.name,
    ...
    fs
      .readFileSync(path.join(extension, "popup.html"), "utf8")
      .replace(/<[^>]*>/g, ""),
  ].join("\n");
  assert.doesNotMatch(
    ownCopy,
    /\bX\b|x\.com/i,
    "visible extension copy must use Twitter, not the renamed brand",
  );
  assert.ok(
    manifest.content_scripts[0].matches.includes("https://x.com/*"),
    "real internal domain matching must remain intact",
  );
